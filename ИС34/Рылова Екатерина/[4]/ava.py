...
class Avatar(turtle.Turtle):
    ...
    def a_star(self, start, target): # делаем метод A звёздочка
        queue = [(0, start, [])]
        used = set()
    ...
    def init_path(self):
        a = int(input("Введите X ключевой точки Аватара: "))
        b = int(input("Введите y ключевой точки Аватара: "))
        self.path = self.a_star((a, b), self.target)[::self._speed]
