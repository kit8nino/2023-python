...
def mob_coords(maze):
    while 1:
        try:
            x = int(input(f"Введите координату X mob'a в пределах от 0 до {len(maze) - 1}):"))
            if 0 <= x < len(maze):
                y = int(input(f"Введите координату Y mob'a в пределах от 0 до {len(maze[0]) - 1}):"))
                if 0 <= y < len(maze[0]):
                    if str(maze[x][y]) == "1":
